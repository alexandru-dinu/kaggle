...
def load_data(sentence_maxlen, shuffle_train=False):
    ...
    if LOCAL and os.path.isfile("glove_embedding_matrix.npy"):
        emb_matrix = np.load("glove_embedding_matrix.npy")
    else:
        emb_matrix = build_glove_embedding_matrix(
            w_idx=word_index, len_voc=len(train_vocab)
        )
        if LOCAL:
            np.save(file="glove_embedding_matrix.npy", arr=emb_matrix)
...
for fold_idx, (train_idx, val_idx) in enumerate(train_splits, start=1):
    ...
    model = Net(
        emb_matrix=EMBEDDING_MATRIX, sen_maxlen=HP["sentence_maxlen"], num_layers=1
    )
    model.cuda()
