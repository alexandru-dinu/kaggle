...
def load_data(sentence_maxlen, shuffle_train=False):
    ...
    e1 = build_embedding_matrix(name='glove', w_idx=word_index, len_voc=len(train_vocab), fill_oov='zeros')
    LOG("->loaded glove")

    e2 = build_embedding_matrix(name='paragram', w_idx=word_index, len_voc=len(train_vocab), fill_oov='zeros')
    LOG("->loaded paragram")
...
train_splits = list(StratifiedKFold(n_splits=HP['num_splits'], shuffle=True, random_state=HP['seed']).split(X_TRAIN, Y_TRAIN))
